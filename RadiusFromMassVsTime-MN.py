...
#gamma = 2.8 #Geometric constant such that sigma_r = r_eff for a spherical clump
Shape = 4 #Choose shape for mass as a function of time, 1 = Sigmoid, 2 = Sphere, 3 = two clumps with sigmoids
...
if Shape == 2 : 
    ...
    r_eff = 1.e6 # Effective spherical radius of total metal mass entering the mantle, in meters
    U = 10.e3  # Velocity of metal entering the mantle, in m/s
    t_max = 2.*r_eff/U # Maximum time, in seconds
    Dt = t_max/1000 # Time step, in seconds
    Time = np.arange(0,t_max+Dt,Dt) # Time vector, in seconds
    z = U*Time #Height, in meters
...
if Shape == 4 : # read from a file
    data = np.loadtxt('Accretion_Ratios/sphg9__01-accRatios_edit.txt', skiprows=1)
    Time_data = data[:, 0] * 3600 # converted into seconds 
...
# Apply the moving average filter
    ...
    Mass = Mass * M_tot #Making the mass dimentional
...
valid_indices = dM_dt >= 0 #removing dM/dt < 0 (this happens at the end. There are some zeros though)
...
def Compute_PDF(M,dM_dt,Dt,Radius,r_min,r_max,Dr) : 
    r = np.arange(r_min,r_max,Dr) #Radius
...
#Function to compute probability density function (pdf) for plume radius 
def Compute_PDF_Shape4(M,dM_dt,Dt,Radius,r_min,r_max,Dr, Mass_tot) : 
    r = np.arange(r_min,r_max,Dr) #Radius
    p = np.zeros(len(r)) # p = pdf
    for i_r in range(len(r)-1):
        X = np.where(Radius>=r[i_r])
